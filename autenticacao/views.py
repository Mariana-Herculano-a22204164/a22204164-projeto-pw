...
def send_mailgun_email(subject, body, to_email):
    ...
    data = urllib.parse.urlencode({
        "from": f"MySite <mailgun@{settings.MAILGUN_DOMAIN}>",
        "to": to_email,
        "subject": subject,
        "html": body
    }).encode()
...
#                         email_subject,
#                         '',
#                         'MySite <mariana@findthemail.com>',
#                         [email],
#                         fail_silently=False,
...
def custom_login(request):
    ...
    return render(request, 'autenticacao/login.html', {'form': form, 'next': request.GET.get('next', '')})
